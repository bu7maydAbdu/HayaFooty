...
module.exports = {
  getLeague: async (req, res) => {
    try {
      const standing = await axios.get(
        `${baseUrl}/${req.params.id}/standings`,
        {
          headers: {
            "X-Auth-Token": token,
          },
      ...
      const currentMatchesSearch = await fixures.data.matches.filter(
  ...
  getChampionsLeague: async (req, res) => {
    try {
      ...
      res.render("cl.ejs", {
        standing: standing.data.standings,
        leagueData: standing.data,
        fixures: currentMatchesSearch,
        playersStats: topScorersData.data.scorers,
      });
